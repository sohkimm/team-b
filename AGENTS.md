...
- 팀명·이름을 알면, 제출용으로 `submit/PROCESS_LOG.md`를 **`팀명_이름_PROCESS_LOG.md`**로 저장(또는 세션 끝에 그 이름으로 복사)해라. 팀별로 개인 로그가 충돌 없이 모인다. (한글 파일명이 압축 시 깨지면 영문도 가능 — 핵심은 팀명·이름이 파일명에 들어가는 것.)
